...
import numpy as np
import os
# lack_pool=['LR', 'JR', 'RS', 'RI', 'WH','PM', 'bb', 'fb','rr','wr']
# parent_1='D:/1分钟数据/'
# path_pool=os.listdir(parent_1)

# for path in path_pool:
#     path_list=os.listdir(parent_1+path)
import re
...
for date_index in range(len(whole_date_list)):
    ...
    #理论长度
    theory_num=len(with_night_0230)
    if target_num>0:
        #小于理论长度开始填充
        
      
        have_time=target_df['time'].values.tolist()
        lack_time=[x for x in with_night_0230 if x not in have_time]
        #检查有没有空的时间
        if lack_time:
        ...
        #缺少时间的个数小于with_night_0230则说明，当天并不是完全没数据,则对主力合约进行填充
        if len(lack_time)<len(with_night_0230):
        ...
        lack_time=[x for x in with_night_0230]
        #一共12列，先全部填充nan的时候，最后再把已知填入
        insert_array=np.empty(shape=(len(lack_time),12))
        insert_array.fill(np.nan)
        insert_df=pd.DataFrame(insert_array,columns=['市场代码','合约代码','时间','开','高','低','收','成交量','成交额','持仓量','date','time'])
...
stop_point=combine_all_df.loc[(combine_all_df['date']=='2019-10-31')&(combine_all_df['time']=='15:00:00')].index[0]
