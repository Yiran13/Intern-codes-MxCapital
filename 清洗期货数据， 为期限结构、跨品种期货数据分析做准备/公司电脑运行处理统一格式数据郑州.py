...
#          修改文件读取路径 read_data
# 郑州特点：
#          两个数据源的格式不统一 ：郑州修改part1  part1.1做了改变
#          合约品种改名的情况：郑州修改part2 做了改变
#          合约代码和其它品种统一问题：郑州代码数字部分只有3位 郑州修改part3
# 
# ===========================================================================================================================================================
# run 文件前必须要明确和修改的变量：
#                                 contract_kind：期货品种
...
# ========================================设置期货品种与交易时间段=========================
for contract_kind in ['CF','AP','CY','CJ','FG','MA','OI','RM','SF','SM','SR','TA','UR','ZC']:
    ...
    data=pd.Series(np.where(data=='None',np.nan,data))
    data_contract=data.str.findall(r'[A-Z]+').str[0]
    data=data_contract+data.str[-7:-4]
...
#                     print(f'{contract_kind} {date} finished!')
    ...
    # ==========================================郑州修改part2================================
    combine_df['合约代码']=combine_df['合约代码'].str.replace('TC','ZC')
    combine_df['合约代码']=combine_df['合约代码'].str.replace('ME','MA')
    combine_df['合约代码']=combine_df['合约代码'].str.replace('RO','OI')
    combine_df=combine_df.sort_values('时间')
    # ===============================================郑州修改part3=======================================================
    # 构造data_date来修改合约代码

    data_date=combine_df[['合约代码','时间']]
    # 分割出品种
    data_date['contract']=data_date.iloc[:,0].str.findall(r'[A-Z]+').str[0]
    #年份从左开始数的倒数第二位，这里是合约交割的年份
    data_date['year_second']=data_date.iloc[:,0].str[-3]
    # 分割出完整年份
    data_date['year']=(data_date.iloc[:,1].str[:4]).astype(float)
    #第二位是0的只能是,2010或者2020
    data_date['year_first']=np.where(np.logical_and(data_date['year']>2011,data_date['year_second']=='0'),'2','1')
    # 有没有可能出现其它的情况？
    # -----------------------------------------------自动检查模块--------------------------------------------------
    data_date_2019=data_date.loc[data_date['year']=='2019']
    check_date=data_date_2019['year_second'].unique().tolist()
    if len(check_date)>2:
        print('年份出错，自动停止，请分析check_date的年份')

    else:
        print('年份检查没问题，开始统一合约代码')
    # 构造合并其它部分
    data_date['others']=data_date.iloc[:,0].str[-3:]
    data_date['contract_code']=data_date['contract']+data_date['year_first']+data_date['others']
    # ----------------------------------------------替换合约代码------------------------------------------------
    combine_df['合约代码']=data_date['contract_code'].values
