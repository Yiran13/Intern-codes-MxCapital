...
#                                 contract_kind：期货品种
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------
#                                 constant_time：每个商品品种交易时间稳定的时间                       
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------
#                                 time_0931_15： 商品期货的交易时间和金融期货早盘和午盘开始的时间不同，不同品种还要考虑有无夜盘,夜盘的时间也有不一致
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
#                                 end_time：     商品期货统一就是15：00，金融期货中的国债期货是15：15  
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
# 解决方法：
#           读取已经做好的统一格式的1min数据
#           统计每个品种在2019/10/31的bar的数量,那一天是周四,也是sample data的最后一个时间点,理论上可以作为正常交易日的bar数量reference
#           确定理论上没有夜盘品种的bar数量(一个固定的数),有夜盘的品种的bar的数量(不止是一个数)
#           可视化不同品种的数量,检查是否有异常值的出现,低于理论或者高于理论的值.有的话,要找出原因.
#           向上滚动验证周一到周三的情况
#           对每个品种在每个交易日打上分钟bar的标签,找到第一次出现符合条件的交易日
#           逐个品种检验,计算在划定的区内符合分钟bar数量的个数占比.
#           对每个商品品种生成一个tuple(期货品种,品种交易时间稳定的首个交易日,品种每日的交易时间,15:00)
# 
# ================================================================================================================================================================
# 变量的相互作用：
...
# ============================读取商品期货交易时间===========================================
time_0931_15=read_pickle('D:/','trading_hours')
# ============================读取金融期货交易时间===========================================
df=pd.read_csv('D:/DIY_主力合约/'+'IF'+'主力连续.csv')
df['time']=df['时间'].str[10:].str.strip()
df['date']=df['时间'].str[:10]
